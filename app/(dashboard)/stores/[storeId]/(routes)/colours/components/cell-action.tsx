...
const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const onCopy = useCallback((id: string) => {
    try {
      ...
      toast.error("Failed to copy colour ID");
    }
  }, []);
  ...
  const onDelete = useCallback(async () => {
    try {
      setIsDeleting(true);
      await axios.delete(`/api/stores/${params?.storeId}/colours/${data.id}`);
      ...
      setTimeout(() => {
        window.location.href = `/stores/${params.storeId}/colours`;
      }, 300);
    } catch (error) {
      console.error("Delete error:", error);
      toast.error("Make sure you removed all products of this colour first.");
    } finally {
      setIsDeleting(false);
    }
  }, [params, data.id]);
        ...
        >
          ...
          <DropdownMenuItem
            ...
            onClick={handleDropdownItemClick(() =>
              handleNavigate(`/stores/${params.storeId}/colours/${data.id}`),
            )}
          >
