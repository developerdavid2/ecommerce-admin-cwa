...
const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const onCopy = useCallback((id: string) => {
    try {
      ...
      toast.error("Failed to copy size ID");
    }
  }, []);
  ...
  const onDelete = useCallback(async () => {
    try {
      ...
      setTimeout(() => {
        window.location.href = `/stores/${params.storeId}/sizes`;
      }, 300);
    } catch (error) {
      console.error("Delete error:", error);
      toast.error("Make sure you removed all products of this size first.");
    } finally {
      setIsDeleting(false);
    }
  }, [params, data.id]);
        ...
        >
          ...
          <DropdownMenuItem
            ...
            onClick={handleDropdownItemClick(() =>
              handleNavigate(`/stores/${params.storeId}/sizes/${data.id}`),
            )}
          >
